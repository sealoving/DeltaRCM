...
Np_sed = 1400; % total number of sediment parcels
...
lambda = 0.5; % "sedimentation lag" - 1.0 means no lag
U_dep_mud = 0.3*u0;
U_ero_sand = 1.05*u0;
U_ero_mud = 1.5*u0;

% topographical diffusion related parameters
alpha = 0.2; %0.05*(0.25*1*0.125); % topo-diffusion coefficient
N_crossdiff = 3; %round(dVs/V0);
...
for i = 1:L
    for j = 1:W
        ...
        elseif type(i,j) == 0 % ocean
            ...
            % by giving a "0" flow rate at a magnitude smaller than channel
...
while timestep < totaltimestep
    ...
    for iter = 1:itermax
        ...
        for np = 1:Np_water
            ...
            iseq(it) = px;
            jseq(it) = py;
            while water_continue == 1 && it < itmax
            ...
            if  sfccredit == 1 && boundflag(iseq(itback),jseq(itback)) == 1
