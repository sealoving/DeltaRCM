...
L = 120; % domain size (# of cells x-direction)
W = 240; % domain size (# of cells y-direction)
CTR = floor(W/2);
...
SLR = 0.12/1000*h0/dt; % sea-level rise per time step
...
for i = 1:L
    for j = 1:W
        if sqrt((i-3)^2+(j-CTR)^2)>= min(L-5,W/2-5)
            boundflag(i,j) = 1;
        end
    end
end
...
z0 = H_SL-h0*strataBtm; % bottom layer ELEVATION
...
%             qs_cap = qs0/u0^beta*U_loc^beta;
    ...
    if mod(timestep,plotinterval) == 0
        ...
        data.uw = uw;
        data.strata = strata;
        save(sprintf('sub%02ddata%d',runID,plotnumber+round(timestep/plotinterval)),'data');
    end
    
end
toc

save(sprintf('sub%02d_final',runID))